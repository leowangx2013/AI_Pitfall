...
from params.params_util import check_paths
...
def set_model_weight_folder(args):
    ...
    for weight in existing_weights:
        ...
        weight_miss_suffix = weight.split("_")[-1]
...
def set_model_weight_file(args):
    ...
    if args.train_mode in {"original", "separate"} or args.stage == "pretrain_classifier":
        if args.weight_folder.endswith(".pt"):
            args.classifier_weight = args.weight_folder
        else:
        ...
        args.detector_weight = os.path.join(
            args.weight_folder,
            f"{args.dataset}_{args.miss_detector}_pretrain_best.pt",
        )
        args.handler_weight = os.path.join(
            args.weight_folder,
            f"{args.dataset}_{args.miss_handler}_pretrain_best.pt",
        )
