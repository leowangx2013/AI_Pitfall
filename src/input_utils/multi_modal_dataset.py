...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if label == 5 or "engine" in os.path.basename(self.sample_files[idx]):
            label = 1
        else:
            label = 0
        # flag = sample["flag"]
        data = sample["data"]
        if not torch.is_tensor(data['shake']['audio']):
            data['shake']['audio'] = torch.from_numpy(data['shake']['audio']).float()
            data['shake']['seismic'] = torch.from_numpy(data['shake']['seismic']).float()

        # data['shake']['audio'] = torch.abs(torch.fft.fft(data['shake']['audio']))
        # data['shake']['seismic'] = torch.abs(torch.fft.fft(data['shake']['seismic']))
